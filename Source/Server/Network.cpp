...
ConnectionsManager::ConnectionsManager(PeerID maxConnectionsCount)
...
{
	if (auto it = m_peerIDs.find(peer); it != m_peerIDs.end()) { return it->second; } 
...
Medae::Encryption::PublicKey ConnectionsManager::getPeerKey(PeerID peerID)
{
...
void FileSender::sendFile(const std::string& path, const Network::Peer& peer)
{
	...
	Compression::Data compressedData {};
	...
	compressedData.content = (new uint8_t[compressedData.size]); // NOLINT
	
	compressedData.content[0] = PATH_SIZE; // NOLINT
	*(reinterpret_cast<uint16_t*>(&compressedData.content[1])) = CHECKSUM_LENGTH; // NOLINT
	
	char ch {};
	uint16_t i = 0;
	while (file.get(ch)) {
		compressedData.content[i + 3] = static_cast<uint8_t>(ch); // NOLINT
		i++;
	}
	
	for (uint16_t i = 0; i < PATH_SIZE; ++i) {
		compressedData.content[FILE_SIZE + i + 3] = static_cast<uint8_t>(path[i]); // NOLINT
	}

	Compression::compress(compressedData);

	Network::Packet packet{
		.content = new uint8_t[compressedData.size + CHECKSUM_LENGTH + 1], // NOLINT
	};

	packet.size = compressedData.size + CHECKSUM_LENGTH + 1;

	packet.content[packet.size - 1] = Network::Codes::FILE_SENDING; // NOLINT
...
void FileSender::sendEncryptedFile(const std::string& path, const Network::Peer& peer, const Encryption::PublicKey& key) {
